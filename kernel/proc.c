...
scheduler(void)
{
  ...
  for (;;) {
    ...
    for (p = proc; p < &proc[NPROC]; p++) {
      ...
      int x = y < 100 ? y : 100;
      p->dp = x > 0 ? x : 0;
      if (p->state == RUNNABLE && p->dp < max_priority) {
        chosen = p;
        max_priority = p->dp;
      }
      release(&p->lock);
...
// Traces system calls made by a program/process
int
trace(int mask) 
{
  struct proc *p = myproc();
...
// Sets the static priority of a process with a given pid and resets niceness to 5 
int 
